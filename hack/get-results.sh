...
cpu_query='rate(container_cpu_usage_seconds_total{name="experiment"}[1m])'
mem_query='avg_over_time(container_memory_usage_bytes{name="experiment"}[1m])'
net_rcv_query='rate(container_network_receive_bytes_total{name="experiment"}[1m])'
net_trmt_query='rate(container_network_transmit_bytes_total{name="experiment"}[1m])'
